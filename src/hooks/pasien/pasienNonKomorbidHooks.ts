...
export const usePasienNonKomorbid = () => {
  ...
  const handleGetAll = async () => {
    try {
      ...
      const { data } = await axios.get("/pasien/pasien-non-komorbid");
      setData(data);
  ...
  const handleCreate = async (body, cb) => {
    try {
      ...
      await axios.post("/pasien/pasien-non-komorbid", body);
      setMessage("Berhasil Tambah/Update Data Rekap Pasien Tanpa Komorbid");
      cb();
    } catch (err) {
      const error = err?.response?.data?.message || err.message;
  ...
  const getInterval = async () => {
    try {
      ...
      const { data } = await axios.get("/pasien/interval/pasien-non-komorbid");
      setStatus(data);
    } catch (err) {
      const error = err?.response?.data?.message || err.message;
  ...
  const setInterval = async () => {
    try {
      setLoadingInterval(true);
      if (status === "off") {
        await axios.post(`/pasien/interval/pasien-non-komorbid/on`);
        setStatus("on");
      } else {
        await axios.post(`/pasien/interval/pasien-non-komorbid/off`);
        setStatus("off");
      }
    } catch (err) {
      const error = err?.response?.data?.message || err.message;
...
export const usePasienNonKomorbidForm = ({ onSubmit }) => {
